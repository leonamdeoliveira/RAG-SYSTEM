...
from pipeline.rag.embeddings.base import EmbeddingProvider
...
class IngestReport:
    ...
    def summary(self) -> str:
        return (
            ...
            f"chunks_upserted={self.chunks_upserted} errors={len(self.errors)}"
        )
    ...
    def run(self) -> IngestReport:
        ...
        for rel in to_ingest:
            ...
            try:
                ...
                sparse = emb.sparse if self.provider.supports_sparse else None
                n = self.store.upsert_chunks(chunks, doc, emb.dense, sparse=sparse)
